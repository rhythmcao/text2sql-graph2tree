...
if not args.testing:
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        for j in range(0, nsamples, step_size):
            ...
            all_results = sql_trans.grammar.order_controller.order_history(hyps, current_batch.ids, all_results)
    ...
    # model.load_state_dict(check_point['model'])
    # prod_results = check_point['order']
    # best_order = sql_trans.grammar.order_controller.compute_best_order(prod_results,
                    # save_path=os.path.join(exp_path, 'order'), verbose=True)
    # sql_trans.grammar.order_controller.use_default_order(best_order)
    # dev_acc = decode('dev', os.path.join(exp_path, 'dev.order'), acc_type='sql', etype='exec', order_method='controller')
    # logger.info('FINAL BEST RESULT with Best Order Dev acc: %.4f' % (dev_acc))
    pickle.dump(all_results, open(os.path.join(exp_path, 'history.bin'), 'wb'))
    pickle.dump(all_prod2fields, open(os.path.join(exp_path, 'order.bin'), 'wb'))
else:
