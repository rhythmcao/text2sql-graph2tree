...
python -u -m torch.distributed.launch --nproc_per_node 2 --nnodes=1 --node_rank=0 --master_addr=localhost --master_port=23456 scripts/spider/train_and_eval_gtol_ddp.py --task $task --dataset 'spider' --db_dir 'data/spider/database' --seed $seed $testing $read_model_path \
