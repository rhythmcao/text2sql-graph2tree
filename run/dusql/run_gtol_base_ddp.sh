...
python -u -m torch.distributed.launch --nproc_per_node 2 --nnodes=1 --node_rank=0 --master_addr=localhost --master_port=21116 scripts/dusql/train_and_eval_gtol_ddp.py --task $task --dataset 'dusql' --db_dir 'data/dusql/db_content.json' --seed $seed --device $device $testing $read_model_path \
