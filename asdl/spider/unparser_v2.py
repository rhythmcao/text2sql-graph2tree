...
class UnParser():
    ...
    @ignore_error
    def unparse(self, sql_ast: AbstractSyntaxTree, db: dict, value_candidates: list, *args, **kargs):
        sql = self.unparse_sql(sql_ast, db, value_candidates, *args, **kargs)
        sql = re.sub(r'\s+', ' ', sql)
        return sql, True

    def unparse_sql(self, sql_ast: AbstractSyntaxTree, db: dict, value_candidates: list, *args, **kargs):
        prod_name = sql_ast.production.constructor.name
        if prod_name != 'Single':
            left_sql = self.unparse_sql_unit(sql_ast[self.grammar.get_field_by_text('sql_unit left_sql_unit')][0].value, db, value_candidates, *args, **kargs)
            right_sql = self.unparse_sql_unit(sql_ast[self.grammar.get_field_by_text('sql_unit right_sql_unit')][0].value, db, value_candidates, *args, **kargs)
            return '%s %s %s' % (left_sql, prod_name.upper(), right_sql)
        else:
            return self.unparse_sql_unit(sql_ast[self.grammar.get_field_by_text('sql_unit sql_unit')][0].value, db, value_candidates, *args, **kargs)

    def unparse_sql_unit(self, sql_ast: AbstractSyntaxTree, db: dict, value_candidates: list, *args, **kargs):
        ...
        from_str = 'FROM ' + self.unparse_from(from_field.value, db, value_candidates, *args, **kargs)
        ...
        if 'Where' in ctr_name:
            ...
            where_str = 'WHERE ' + self.unparse_condition(where_field.value, db, value_candidates, 'where', *args, **kargs)
        if 'GroupBy' in ctr_name:
            groupby_field = sql_ast[self.grammar.get_field_by_text('groupby groupby')][0]
            groupby_str = 'GROUP BY ' + self.unparse_groupby(groupby_field.value, db, value_candidates, *args, **kargs)
        if 'OrderBy' in ctr_name:
            orderby_field = sql_ast[self.grammar.get_field_by_text('orderby orderby')][0]
            orderby_str = 'ORDER BY ' + self.unparse_orderby(orderby_field.value, db, value_candidates, *args, **kargs)
    ...
    def unparse_from(self, from_ast: AbstractSyntaxTree, db: dict, value_candidates: list, *args, **kargs):
        ...
        if 'Table' in ctr_name:
            ...
            return '( ' + self.unparse_sql(from_ast[self.grammar.get_field_by_text('sql from_sql')][0].value, db, value_candidates, *args, **kargs) + ' )'

    def unparse_from_condition(self, cond_ast, db, *args, **kargs):
        cond_fields = cond_ast[self.grammar.get_field_by_text('join join')]
        cond_str = []
    ...
    def unparse_groupby(self, groupby_ast: AbstractSyntaxTree, db: dict, value_candidates: list, *args, **kargs):
        ...
        if 'Having' in ctr_name:
            ...
            having_str = ' HAVING ' + self.unparse_condition(having_field.value, db, value_candidates, 'having', *args, **kargs)
        return groupby_str + having_str

    def unparse_orderby(self, orderby_ast: AbstractSyntaxTree, db: dict, value_candidates: list, *args, **kargs):
        col_names, limit_str = [], ''
        ...
        if 'Limit' in ctr_name:
            ...
            limit_str = ' LIMIT ' + self.value_processor.postprocess_value(value_id, value_candidates, db, state)
        return orderby_str + ' ' + order + limit_str

    def unparse_condition(self, conds_ast: AbstractSyntaxTree, db: dict, value_candidates: list, clause: str, *args, **kargs):
        ...
        if 'AndCondition' in ctr_name or 'OrCondition' in ctr_name:
            ...
            for cond_field in cond_fields:
                cond = self.unparse_condition(cond_field.value, db, value_candidates, clause, *args, **kargs)
                conds.append(cond)
    ...
    def unparse_cond(self, cond_ast: AbstractSyntaxTree, db: dict, value_candidates: list, clause: str, *args, **kargs):
        CMP_OP_MAPPING = {
        ...
        if 'Between' in ctr_name:
            ...
            value_str1 = self.unparse_value(val_field.value, db, value_candidates, state, *args, **kargs)
            val_field = cond_ast[self.grammar.get_field_by_text('value right_value')][0]
            value_str2 = self.unparse_value(val_field.value, db, value_candidates, state, *args, **kargs)
            ...
            value_str = self.unparse_value(val_field.value, db, value_candidates, state, *args, **kargs)
        return ' '.join([col_name, cmp_op, value_str])

    def unparse_value(self, val_ast: AbstractSyntaxTree, db: dict, value_candidates: list, state: State, *args, **kargs):
        ...
        if ctr_name == 'LiteralValue':
            ...
            val_str = self.value_processor.postprocess_value(val_id, value_candidates, db, state)
        elif ctr_name == 'SQLValue':
            val_field = val_ast[self.grammar.get_field_by_text('sql value_sql')][0]
            val_str = '( ' + self.unparse_sql(val_field.value, db, value_candidates, *args, **kargs) + ' )'
        else:
