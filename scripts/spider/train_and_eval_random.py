...
train_dataset, dev_dataset = Example.load_dataset('train', shuffle=False), Example.load_dataset('dev')
...
def decode(choice, output_path, acc_type='sql', etype='exec', value_fscore=False, schema_acc=False, order_method='all'):
    ...
    with torch.no_grad():
        ...
        acc = evaluator.acc(all_hyps, all_vals, dataset, output_path, acc_type=acc_type, etype=etype)
        if value_fscore:
...
def gather_orders_from_asts(model, dataset):
    nsamples, step_size = len(dataset), args.batch_size
    results = None
    model.train()
    with torch.no_grad():
        for j in range(0, nsamples, step_size):
            current_batch = Batch.from_example_list(dataset[j: j + step_size], device, train=True, method='gtol')
...
def gather_history_from_asts(model, dataset):
    nsamples, step_size = len(dataset), args.batch_size * 2
    results = None
    model.train()
    with torch.no_grad():
        for j in range(0, nsamples, step_size):
            current_batch = Batch.from_example_list(dataset[j: j + step_size], device, train=True, method='gtol')
            (_, hyps), _, _ = model(current_batch, sample_size=0, gtol_size=args.beam_size, n_best=1, order_method='all', method='gtol')
            results = sql_trans.grammar.order_controller.order_history(hyps, current_batch.ids, results)
    return results
...
if not args.testing:
    ...
    for i in range(start_epoch, args.max_epoch):
        start_time = time.time()
        epoch_loss, epoch_vr_loss, epoch_gp_loss, count = 0, 0, 0, 0
        ...
        for j in range(0, nsamples, step_size):
            ...
            current_batch = Batch.from_example_list(cur_dataset, device, train=True, method='random', typed_random=True, untyped_random=True, smoothing=args.smoothing)
            loss, vr_loss, gp_loss = model(current_batch, sample_size, method='random') # see utils/batch.py for batch elements
        ...
        # all_history.append(gather_history_from_asts(model, train_dataset))

        if i < args.eval_after_epoch: # avoid unnecessary evaluation
            continue

        start_time = time.time()
        dev_acc = decode('dev', os.path.join(exp_path, 'dev.iter' + str(i)), acc_type='sql', etype='exec', order_method='all')
    ...
    # pickle.dump(all_history, open(os.path.join(exp_path, 'history.bin'), 'wb'))
    check_point = torch.load(open(os.path.join(exp_path, 'model.bin'), 'rb'), map_location=device)
    model.load_state_dict(check_point['model'])
    results = gather_orders_from_asts(model, train_dataset)
    prod2fields = sql_trans.grammar.order_controller.compute_best_order(results, save_path=os.path.join(exp_path, 'order.bin'), verbose=True)
    sql_trans.grammar.order_controller.set_order(prod2fields)
    dev_acc = decode('dev', os.path.join(exp_path, 'dev.order'), acc_type='sql', etype='exec', order_method='controller')
    logger.info('FINAL BEST RESULT Using Best Order from Training Dataset: \tDev acc: %.4f' % (dev_acc))
else:
    ...
    dev_acc = decode('dev', os.path.join(exp_path, 'dev.order'), acc_type='sql', etype='exec', order_method='controller')
    logger.info('FINAL BEST RESULT Using Best Order from Training Dataset: \tDev acc: %.4f' % (dev_acc))
    # (em_acc, ex_acc), value_fscore, schema_acc = decode('dev', output_path=os.path.join(args.read_model_path, 'dev.eval' + str(args.beam_size)), acc_type='eg-sql', etype='all', value_fscore=True, schema_acc=True)
    ...
    # start_time = time.time()
    # ex_acc_beam = decode('dev', output_path=os.path.join(args.read_model_path, 'dev.eval.beam' + str(args.beam_size)), acc_type='beam', etype='exec')
