...
from utils.constants import UNK, SCHEMA_TYPES, RELATIONS, DEBUG, GRAMMAR_FILEPATH
...
class Example():
    ...
    @classmethod
    def configuration(cls, plm=None, method='lgesql', table_path='data/spider/tables.json', tables='data/spider/tables.bin', db_dir='data/spider/database', order_path=None):
        cls.dataset = 'spider'
    ...
    def load_dataset(cls, choice, order_seed=999, shuffle=False):
        ...
        fp = os.path.join('data', 'spider', choice + '.bin') if not DEBUG else os.path.join('data', 'spider', 'train.bin')
        ...
        for idx, ex in enumerate(datasets):
    ...
    def __init__(self, ex: dict, db: dict, shuffle: bool = False, id: int = -1, fixed=False):
        ...
        if Example.plm is None:
            ...
            self.column = [[db['column_types'][idx].lower()] + c + ex['processed_cells'][idx]
            ...
            self.column = [[db['column_types'][idx].lower()] + c.lower().split() + ex['cells'][idx] for idx, (_, c) in enumerate(db['column_names'])]
