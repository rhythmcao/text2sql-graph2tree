...
def gather_orders_from_asts(model, dataset):
    ...
    with torch.no_grad():
        for j in range(0, nsamples, step_size):
            cur_dataset = dataset[j: j + step_size]
            current_batch = Batch.from_example_list(cur_dataset, device, train=True, method='gtol')
...
if not args.testing:
    ...
    dev_acc = decode('dev', os.path.join(exp_path, 'dev.order'), acc_type='sql', order_method='controller')
    ...
    # logger.info("Evaluation costs %.2fs ; Dev dataset execution-guided exact match acc with value is %.4f ." % (time.time() - start_time, acc))
    ...
    # start_time = time.time()
    # beam_acc = decode('dev', output_path=os.path.join(args.read_model_path, 'dev.eval.beam' + str(args.beam_size)), acc_type='beam')
    # logger.info("Evaluation costs %.2fs ; Dev dataset exact match acc with value (any one in the beam is correct) is %.4f ." % (time.time() - start_time, beam_acc))
