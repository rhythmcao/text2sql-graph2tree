...
def decode(choice, output_path, acc_type='sql', etype='exec', value_fscore=False, schema_acc=False, order_method='controller'):
...
if not args.testing:
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        start_time = time.time()
        dev_acc = decode('dev', os.path.join(exp_path, 'dev.iter' + str(i)), acc_type='sql', etype='exec', order_method='controller')
