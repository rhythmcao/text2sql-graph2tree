...
if not args.testing:
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        if is_master: # only evaluate on the master GPU
            start_time = time.time()
            em, ex = decode(base_model, evaluator, dev_dataset, os.path.join(exp_path, 'dev.iter' + str(i)), batch_size=args.test_batch_size, beam_size=args.beam_size,
                ts_order=('controller' if args.ts_order == 'controller' else 'enum'), acc_type='eg-sql', etype='all', device=device)['sql']
            dev_acc = (em + ex) / 2.0 if Example.dataset == 'spider' else ex
...
if is_master:
    ...
    start_time = time.time()
    results = decode(base_model, evaluator, dev_dataset, os.path.join(exp_path, 'dev.eval.enum'), batch_size=args.test_batch_size, beam_size=args.beam_size,
        ts_order='enum', acc_type='eg-sql', etype='all', value_metric='all', schema_metric='all', device=device)
    ...
    start_time = time.time()
    results = decode(base_model, evaluator, dev_dataset, os.path.join(exp_path, 'dev.eval.controller'), batch_size=args.test_batch_size, beam_size=args.beam_size,
        ts_order='controller', acc_type='eg-sql', etype='all', device=device)
    logger.info(f"EVALUATION costs {time.time() - start_time:.2f}s")
    logger.info(f"EVALUATION: \tDev sql exact set match/execution acc: {results['sql']}")
