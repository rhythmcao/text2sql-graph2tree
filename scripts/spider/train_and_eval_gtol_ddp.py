...
from utils.spider.example import Example, DatasetWrapper
...
if not args.testing:
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        if is_master(): # only evaluate on the master GPU
            start_time = time.time()
            dev_acc = decode('dev', os.path.join(exp_path, 'dev.iter' + str(i)), acc_type='sql', etype='exec', order_method='all')#args.order_method)
    ...
    if is_master():
        ...
        start_time = time.time()
        (em_acc, ex_acc), value_fscore, schema_acc = decode('train', output_path=os.path.join(args.read_model_path, 'train.eval' + str(args.beam_size)), acc_type='eg-sql', etype='all', value_fscore=True, schema_acc=True)
