...
train_dataset, dev_dataset = Example.load_fixed_dataset('train', args.order_seed, True), Example.load_dataset('dev')
...
def decode(choice, output_path, acc_type='sql', etype='exec', value_fscore=False, schema_acc=False):
    ...
    with torch.no_grad():
        for i in range(0, len(dataset), args.batch_size):
            ...
            hyps, vals, gates = model.parse(current_batch, args.beam_size, order_method='controller')
...
if not args.testing:
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        start_time = time.time()
        dev_acc = decode('dev', os.path.join(exp_path, 'dev.iter' + str(i)), acc_type='sql', etype='exec')
