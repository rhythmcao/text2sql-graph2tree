...
class OutputProcessor(BaseOutputProcessor):
    ...
    def __init__(self, table_path=None, db_dir=None, **kargs) -> None:
        ...
        self.predict_value = DATASETS['cspider']['value']
    ...
    def pipeline(self, entry: dict, db: dict, verbose: bool = False):
        ...
        if self.predict_value:
            ...
            entry = super(OutputProcessor, self).auxiliary_labels(entry, db)
        else:
            entry['candidates'], entry['values'] = [], set()
    ...
    def auxiliary_labels(self, entry: dict, db: dict):
        graph = entry['graph']
        t_num, s_num = len(db['table_names']), len(db['table_names']) + len(db['column_names'])
        def check_node(i):
