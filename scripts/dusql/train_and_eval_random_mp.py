...
def gather_history_from_asts(model, dataset):
    nsamples, step_size = len(dataset), args.batch_size * 5
    results = None
    model.train()
    with torch.no_grad():
        for j in range(0, nsamples, step_size):
...
if not args.testing:
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        for j, cur_dataset in enumerate(train_loader):
            ...
            current_batch = Batch.from_example_list(cur_dataset, device, train=True, method='random', typed_random=False, untyped_random=True, smoothing=args.smoothing)

            if count == args.grad_accumulate or j == last_iter:
                loss, vr_loss, gp_loss = model(current_batch, sample_size=sample_size, method='random')
        ...
        if is_master():
            ...
            # all_history.append(gather_history_from_asts(model.module, train_dataset.examples))
    ...
    if is_master():
        ...
        # model.module.load_state_dict(check_point['model'])
        # results = gather_orders_from_asts(model.module, train_dataset.examples)
