...
################################
# val: number(float)/string(str)/sql(dict)/col_id(int)
...
################################
# db_content.json
# "db_id": database id or name
# "tables": (dict)
#   [table_name]: (dict) {
#       "cell": (list) [
#           [xxx, xxx, xxx, xxx], # one row
#           [xxx, xxx, xxx, xxx], # one row
#           ...
#       ],
#       "header": (list of string) [column_name1, col_name2, col_name3, col_name4],
#       "type": (list of string) ["time", "number", "number", "text"]
#       "table_name": "xxxx"
#   }
################################
...
def count_agg(units): # col_unit or (agg_id, val_unit)
...
def count_component1(sql):
    ...
    count += len([cond_unit for cond_unit in cond_units if cond_unit[1] == COND_OPS.index('like')])
...
def count_others(sql):
    ...
    where = [cond[2][1] for cond in sql['where'][::2]]
    agg_count += count_agg(where)
    agg_count += count_agg(sql['groupBy'])
    if len(sql['orderBy']) > 0:
        agg_count += count_agg(sql['orderBy'][1])
    having = [cond[2][1] for cond in sql['having'][::2]]
    agg_count += count_agg(having)
    if agg_count > 1:
        count += 1
...
def evaluate_complex(table, gold, predict, single_equal=False, verbose=True):
    """evaluate main
    Args:
    ...
    partial_types = ['select', 'select(no AGG)', 'where', 'where(no OP)', 'group',
                     'having', 'order', 'and/or', 'IUEN', 'keywords']
    scores = {}
    for level in levels:
        scores[level] = {'count': 0, 'partial': {}, 'exact': 0., 'exact(no value)': 0., 'partial(no value)': {}}
        for type_ in partial_types:
            scores[level]['partial'][type_] = {'acc': 0., 'rec': 0., 'f1': 0.,'acc_count':0,'rec_count':0}
            # scores[level]['partial(no value)'][type_] = {'acc': 0., 'rec': 0., 'f1': 0.,'acc_count':0,'rec_count':0}
    ...
    for ins_id, g in gold_dict.items():
        ...
        if exact_score == 0 and verbose:
            # print("gold sql: %s" % (json.dumps(gold_sql, ensure_ascii=False)))
            # print("pred sql: %s" % (json.dumps(pred_sql, ensure_ascii=False)))
            print("error instance %s: %s\npred: %s\ngold: %s\n" % (ins_id, hardness, pred_str, gold_str))
        scores['all']['count'] += 1
        scores['all']['exact'] += exact_score
        scores['all']['exact(no value)'] += exact_score_novalue
        scores[hardness]['exact'] += exact_score
        scores[hardness]['exact(no value)'] += exact_score_novalue

        # partial scores
        score = evaluator.eval_partial_match(pred_sql, gold_sql, value_match=True)
        for type_ in partial_types:
            if score[type_]['pred_total'] > 0:
                scores[hardness]['partial'][type_]['acc'] += score[type_]['acc']
                scores[hardness]['partial'][type_]['acc_count'] += 1
                scores['all']['partial'][type_]['acc'] += score[type_]['acc']
                scores['all']['partial'][type_]['acc_count'] += 1
            if score[type_]['gold_total'] > 0:
                scores[hardness]['partial'][type_]['rec'] += score[type_]['rec']
                scores[hardness]['partial'][type_]['rec_count'] += 1
                scores['all']['partial'][type_]['rec'] += score[type_]['rec']
                scores['all']['partial'][type_]['rec_count'] += 1
            scores[hardness]['partial'][type_]['f1'] += score[type_]['f1']
            scores['all']['partial'][type_]['f1'] += score[type_]['f1']

    for level in levels:
        if scores[level]['count'] == 0:
            continue
        scores[level]['exact'] /= scores[level]['count']
        scores[level]['exact(no value)'] /= scores[level]['count']
        # partial scores
        for type_ in partial_types:
...
def evaluate(table, gold, predict, dataset='DuSQL', verbose=True):
    """
    dataset:['CSpider', 'DuSQL', 'NL2SQL']
    """
    if dataset == 'NL2SQL':
        scores, scores_novalue = evaluate_NL2SQL(table, gold, predict, single_equal=True, verbose=verbose)
    elif dataset == 'DuSQL':
        scores, scores_novalue = evaluate_complex(table, gold, predict, single_equal=True, verbose=verbose)
    else:
        scores, scores_novalue = evaluate_complex(table, gold, predict, single_equal=True, verbose=verbose)
...
def print_scores(scores):
    ...
    for type_ in partial_types:
        this_scores = [scores[level][partition][type_]['rec'] for level in levels]
        print_formated_s(type_, this_scores, '{:<20.3f}')

    print ('---------------------- PARTIAL MATCHING F1 --------------------------')
    for type_ in partial_types:
        this_scores = [scores[level][partition][type_]['f1'] for level in levels]
        print_formated_s(type_, this_scores, '{:<20.3f}')
