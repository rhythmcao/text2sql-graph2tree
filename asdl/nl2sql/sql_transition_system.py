...
class SQLTransitionSystem(TransitionSystem):
    def __init__(self, grammar, table_path=None, db_dir=None):
        ...
        from asdl.nl2sql.parser import Parser
        from asdl.nl2sql.unparser import UnParser
        if table_path is None:
            table_path = os.path.join(DATASETS['nl2sql']['data'], 'tables.bin')
        if db_dir is None:
...
if __name__ == '__main__':
    ...
    def evaluate_sqls(recovered_sqls, choice='train'):
        ...
        with open(output_path, 'w') as of:
            ...
            evaluate(table_path, gold_path, pred_path, dataset='NL2SQL')
