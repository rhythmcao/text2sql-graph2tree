...
class LockedDropout(nn.Module):
    ...
    def forward(self, x, layer=0, prev_ids=None):
        ...
        if prev_ids is None:
            ...
            mask = mask[prev_ids].expand_as(x)
        return mask * x
...
class RecurrentNeuralNetwork(nn.Module):
    ...
    def forward(self, inputs, hiddens=None, start=False, prev_ids=None, layerwise=False):
        """
        @args:
        ...
        for l in range(len(self.cells)):
            ...
            prev_layer = self.locked_dropout(prev_layer, layer=l, prev_ids=prev_ids)
