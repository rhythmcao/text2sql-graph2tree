...
def from_example_list_base(ex_list, device='cpu', train=True):
    ...
    if plm is None: # glove.42B.300d
        ...
        position_ids = [get_position_ids(ex, shuffle=train, add_one=(Example.dataset == 'dusql')) + [0] * (max_len - len(ex.input_id)) for ex in ex_list]
...
class Batch():
    ...
    def get_frontier_prod_idx(self, t):
        ids = [Example.grammar.prod2id[a[t].frontier_prod] if t < len(a) else 0 for a in self.tgt_actions]
        return torch.tensor(ids, dtype=torch.long, device=self.device)

    def get_frontier_field_idx(self, t):
        ids = [Example.grammar.field2id[a[t].frontier_field] if t < len(a) else 0 for a in self.tgt_actions]
        return torch.tensor(ids, dtype=torch.long, device=self.device)

    def get_frontier_type_idx(self, t):
        ids = [Example.grammar.type2id[a[t].frontier_type] if t < len(a) else 0 for a in self.tgt_actions]
        return torch.tensor(ids, dtype=torch.long, device=self.device)

    def get_parent_state_idx(self, t):
        return [a[t].parent_t if t < len(a) else 0 for a in self.tgt_actions]
