...
python -u scripts/dusql/train_and_eval_random_mp.py --task $task --dataset 'dusql' --db_dir 'data/dusql/db_content.json' --seed $seed $testing $read_model_path $read_order_path \
    --local_rank 0 --rank $rank --world_size $world_size \
