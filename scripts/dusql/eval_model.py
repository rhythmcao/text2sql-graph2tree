...
if False: # for fixed order and rl models
...
def decode(output_path, order_method='controller', add_auxiliary=True):
    ...
    exec_acc, match_acc = evaluator.acc(all_hyps, all_vals, dev_dataset, output_path, acc_type='sql', etype='all')
    ex, em = evaluator.acc(all_hyps, all_vals, dev_dataset, output_path, acc_type='eg-sql', etype='all')
    if add_auxiliary:
