...
if not args.testing:
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        model.train()
        # shuffle ts order for each training sample, such that each beam has the same ts order for each training sample
        train_dataset = controller.set_canonical_order_per_sample(train_dataset, ts_control=False, ts_shuffle=True, uts_shuffle=False)
        for j, cur_dataset in enumerate(train_loader):
            ...
            current_batch = Batch.from_example_list(cur_dataset, device, train=True, ts_order='controller', uts_order='enum', smoothing=args.smoothing)

            update_flag = count == args.grad_accumulate or j == last_iter
            cntx = model.no_sync() if args.ddp and not update_flag else nullcontext()
            with cntx:
                outputs = model(current_batch, sample_size=sample_size, gtl_size=args.gtl_size, n_best=args.n_best, ts_order='controller', uts_order='enum')
